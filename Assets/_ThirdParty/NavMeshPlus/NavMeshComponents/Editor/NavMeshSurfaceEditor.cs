...
namespace NavMeshPlus.Editors.Components
{
    ...
    class NavMeshSurfaceEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (!targets.Contains(bakeOperations[i].Surface2D))
                    continue;
        ...
        static void RenderBoxGizmoSelected(NavMeshSurface2D navSurface2D, GizmoType gizmoType)
        {
            RenderBoxGizmo(navSurface2D, gizmoType, true);
        }

        [DrawGizmo(GizmoType.NotInSelectionHierarchy | GizmoType.Pickable)]
        static void RenderBoxGizmoNotSelected(NavMeshSurface2D navSurface2D, GizmoType gizmoType)
        {
            if (NavMeshVisualizationSettings.showNavigation > 0)
                RenderBoxGizmo(navSurface2D, gizmoType, false);
            else
                Gizmos.DrawIcon(navSurface2D.transform.position, "NavMeshSurface Icon", true);
        }

        static void RenderBoxGizmo(NavMeshSurface2D navSurface2D, GizmoType gizmoType, bool selected)
        {
            var color = selected ? s_HandleColorSelected : s_HandleColor;
            if (!navSurface2D.enabled)
            ...
            var localToWorld = Matrix4x4.TRS(navSurface2D.transform.position, navSurface2D.transform.rotation, Vector3.one);
            Gizmos.matrix = localToWorld;

            if (navSurface2D.collectObjects == CollectObjects.Volume)
            {
                Gizmos.color = color;
                Gizmos.DrawWireCube(navSurface2D.center, navSurface2D.size);

                if (selected && navSurface2D.enabled)
                {
                    var colorTrans = new Color(color.r * 0.75f, color.g * 0.75f, color.b * 0.75f, color.a * 0.15f);
                    Gizmos.color = colorTrans;
                    Gizmos.DrawCube(navSurface2D.center, navSurface2D.size);
                }
            }
            else
            {
                if (navSurface2D.navMeshData != null)
                {
                    var bounds = navSurface2D.navMeshData.sourceBounds;
