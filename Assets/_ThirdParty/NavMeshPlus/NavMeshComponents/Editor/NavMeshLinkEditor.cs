...
namespace NavMeshPlus.Editors.Components
{
    ...
    class NavMeshLinkEditor : Editor
    {
        ...
        void AlignTransformToEndPoints(NavMeshLink2D navLink2D)
        {
            var mat = UnscaledLocalToWorldMatrix(navLink2D.transform);

            var worldStartPt = mat.MultiplyPoint(navLink2D.startPoint);
            var worldEndPt = mat.MultiplyPoint(navLink2D.endPoint);

            var forward = worldEndPt - worldStartPt;
            var up = navLink2D.transform.up;

            // Flatten
            forward -= Vector3.Dot(up, forward) * up;

            var transform = navLink2D.transform;
            transform.rotation = Quaternion.LookRotation(forward, up);
            transform.position = (worldEndPt + worldStartPt) * 0.5f;
            transform.localScale = Vector3.one;

            navLink2D.startPoint = transform.InverseTransformPoint(worldStartPt);
            navLink2D.endPoint = transform.InverseTransformPoint(worldEndPt);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                foreach (NavMeshLink2D navLink in targets)
                {
        ...
        static Vector3 CalcLinkRight(NavMeshLink2D navLink2D)
        {
            var dir = navLink2D.endPoint - navLink2D.startPoint;
            return (new Vector3(-dir.z, 0.0f, dir.x)).normalized;
        }

        static void DrawLink(NavMeshLink2D navLink2D)
        {
            var right = CalcLinkRight(navLink2D);
            var rad = navLink2D.width * 0.5f;

            Gizmos.DrawLine(navLink2D.startPoint - right * rad, navLink2D.startPoint + right * rad);
            Gizmos.DrawLine(navLink2D.endPoint - right * rad, navLink2D.endPoint + right * rad);
            Gizmos.DrawLine(navLink2D.startPoint - right * rad, navLink2D.endPoint - right * rad);
            Gizmos.DrawLine(navLink2D.startPoint + right * rad, navLink2D.endPoint + right * rad);
        }

        [DrawGizmo(GizmoType.Selected | GizmoType.Active | GizmoType.Pickable)]
        static void RenderBoxGizmo(NavMeshLink2D navLink2D, GizmoType gizmoType)
        {
            if (!EditorApplication.isPlaying)
                navLink2D.UpdateLink();

            var color = s_HandleColor;
            if (!navLink2D.enabled)
                color = s_HandleColorDisabled;

            var oldColor = Gizmos.color;
            var oldMatrix = Gizmos.matrix;

            Gizmos.matrix = UnscaledLocalToWorldMatrix(navLink2D.transform);
            ...
            Gizmos.DrawIcon(navLink2D.transform.position, "NavMeshLink Icon", true);
        }

        [DrawGizmo(GizmoType.NotInSelectionHierarchy | GizmoType.Pickable)]
        static void RenderBoxGizmoNotSelected(NavMeshLink2D navLink2D, GizmoType gizmoType)
        {
        ...
        {
            var navLink = (NavMeshLink2D)target;
            if (!navLink.enabled)
                return;
