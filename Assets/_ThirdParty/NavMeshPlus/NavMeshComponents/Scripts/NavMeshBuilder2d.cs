...
namespace NavMeshPlus.Extensions
{
    ...
    class NavMeshBuilder2d
    {
        ...
        {
            foreach (var modifier in root.GetComponentsInChildren<NavMeshModifier2D>())
            {
        ...
        public static void CollectSources(List<NavMeshBuildSource> sources, NavMeshBuilder2dState builder, NavMeshModifier2D modifier2D, int area)
        {
            if (builder.CollectGeometry == NavMeshCollectGeometry.PhysicsColliders)
            {
                var collider = modifier2D.GetComponent<Collider2D>();
                if (collider != null)
                {
            ...
            {
                var tilemap = modifier2D.GetComponent<Tilemap>();
                if (tilemap != null)
                {
                    CollectTileSources(sources, tilemap, area, builder);
                }
                var sprite = modifier2D.GetComponent<SpriteRenderer>();
                if (sprite != null)
                {
        ...
        private static void AddDefaultWalkableTilemap(List<NavMeshBuildSource> sources, NavMeshBuilder2dState builder, NavMeshModifier2D modifier2D)
        {
            var tilemap = modifier2D.GetComponent<Tilemap>();
            if (tilemap != null)
            {
        ...
        public static void CollectTileSources(List<NavMeshBuildSource> sources, Tilemap tilemap, int area, NavMeshBuilder2dState builder)
        {
            ...
            var grid = FindClosestParentGrid(tilemap.transform);
            var size = new Vector3(grid.cellSize.x, grid.cellSize.y, 0);
        ...
        {
            return Matrix4x4.TRS(Vector3.Scale(tilemap.GetCellCenterWorld(vec3int), scale) - FindClosestParentGrid(tilemap.transform).cellGap, tilemap.transform.rotation, tilemap.transform.lossyScale) * tilemap.orientationMatrix * tilemap.GetTransformMatrix(vec3int);
        }
        ...
        public static Grid FindClosestParentGrid(Transform tr)
        {
            Transform currentTransform = tr.parent;

            while (currentTransform != null)
            {
                Grid component = currentTransform.GetComponent<Grid>();
                if (component != null)
                {
                    return component;
                }
