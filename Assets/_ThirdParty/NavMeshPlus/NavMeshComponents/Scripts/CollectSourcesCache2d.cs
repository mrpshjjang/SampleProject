...
namespace NavMeshPlus.Extensions
{
    ...
    public class CollectSourcesCache2d : NavMeshExtension
    {
        ...
        {
            IsDirty = false;
            return NavMeshBuilder.UpdateNavMeshDataAsync(data, NavMeshSurface2DOwner.GetBuildSettings(), _sources, _sourcesBounds);
        }
        public AsyncOperation UpdateNavMesh()
        {
            return UpdateNavMesh(NavMeshSurface2DOwner.navMeshData);
        }
        public override void CollectSources(NavMeshSurface2D surface2D, List<NavMeshBuildSource> sources, NavMeshBuilderState navMeshState)
        {
            _lookup.Clear();
            IsDirty = false;
            _state?.Dispose();
        ...
        public override void PostCollectSources(NavMeshSurface2D surface2D, List<NavMeshBuildSource> sources, NavMeshBuilderState navNeshState)
        {
