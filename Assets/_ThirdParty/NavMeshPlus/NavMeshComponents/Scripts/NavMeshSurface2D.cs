...
namespace NavMeshPlus.Components
{
    ...
    public class NavMeshSurface2D : MonoBehaviour
    {
        ...
        static void Register(NavMeshSurface2D surface2D)
        {
#if UNITY_EDITOR
            var isInPreviewScene = EditorSceneManager.IsPreviewSceneObject(surface2D);
            var isPrefab = isInPreviewScene || EditorUtility.IsPersistent(surface2D);
            if (isPrefab)
            {
                //Debug.LogFormat("NavMeshData from {0}.{1} will not be added to the NavMesh world because the gameObject is a prefab.",
            ...
            if (!s_NavMeshSurfaces.Contains(surface2D))
                s_NavMeshSurfaces.Add(surface2D);
        }

        static void Unregister(NavMeshSurface2D surface2D)
        {
            s_NavMeshSurfaces.Remove(surface2D);
        ...
        List<NavMeshBuildSource> CollectSources(NavMeshBuilderState builderState)
        {
            ...
            List<NavMeshModifier2D> modifiers;
            if (m_CollectObjects == CollectObjects.Children)
            {
                modifiers = new List<NavMeshModifier2D>(GetComponentsInChildren<NavMeshModifier2D>());
                modifiers.RemoveAll(x => !x.isActiveAndEnabled);
            }
            else
            {
                modifiers = NavMeshModifier2D.activeModifiers;
            }
