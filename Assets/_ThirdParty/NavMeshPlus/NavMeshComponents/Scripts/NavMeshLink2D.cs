...
namespace NavMeshPlus.Components
{
    ...
    public class NavMeshLink2D : MonoBehaviour
    {
        ...
        static readonly List<NavMeshLink2D> s_Tracked = new List<NavMeshLink2D>();
        ...
        static void AddTracking(NavMeshLink2D link2D)
        {
#if UNITY_EDITOR
            if (s_Tracked.Contains(link2D))
            {
                Debug.LogError("Link is already tracked: " + link2D);
                return;
            }
#endif
            ...
            s_Tracked.Add(link2D);
        }

        static void RemoveTracking(NavMeshLink2D link2D)
        {
            s_Tracked.Remove(link2D);
